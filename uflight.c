...
void make_city_buildings(int x, int y, int size, gameState *gs) {
	...
	for (t = 0; t < 100; t++) {
		...
		switch (t % 5) {
		case 0:
			bheight = abs(mrandom(200)) + 350;
			segments = bheight / 100;
			makebox(orange0, lx, ly, lz, abs(mrandom(200)) + 100, bheight, abs(mrandom(200)) + 300, segments, gs);
			break;
		case 1:
			bheight = abs(mrandom(200)) + 250;
			segments = bheight / 100;
			makebox(tan0, lx, ly, lz, abs(mrandom(200)) + 250, bheight, abs(mrandom(200)) + 400, segments, gs);
			break;
		case 2:
			bheight = abs(mrandom(200)) + 150;
			segments = bheight / 100;
			makebox(bgrey+5, lx, ly, lz, abs(mrandom(200)) + 350, bheight, abs(mrandom(400)) + 250, segments, gs);
			break;
		case 3:
...
void make_buildings(int eye_x,int eye_y,int eye_z, gameState* gs)
{
	...
	{
		static const int truck1x[] = { -4000, -4000, 8000, 8000, 16000, 16000, 4000, 4000 };
		static const int truck1z[] = { -52000, -40000, -40000, -28000, -28000, -16000, -16000, -52000 };
		static const int truck2x[] = { -28000, -28000, -16000, -16000, -4000, -4000, -28000 };
		static const int truck2z[] = { -52000, -36000, -36000, -24000, -24000, -12000, -12000 };
		static const int truck3x[] = { 12000, 12000, 24000, 24000, 36000, 36000, 12000 };
		static const int truck3z[] = { -76000, -56000, -56000, -36000, -36000, -16000, -16000 };
		set_truck_route(truck1x[0], 0, truck1z[0], 1.2f, 8, truck1x, truck1z, gs);
		set_truck_route(truck2x[0], 0, truck2z[0], 1.0f, 7, truck2x, truck2z, gs);
		set_truck_route(truck3x[0], 0, truck3z[0], 0.9f, 7, truck3x, truck3z, gs);
	}
...
void draw_buildings(float rx, float ry, float rz, int start_plane, int num_planes, gameState* gs) {
	...
	for (i = 0; i < current_building; i++) {
		...
		case TYPE_MOUNTAIN:
			...
			break;
		case TYPE_GROUND_VEHICLE:
			glPushMatrix();
			glTranslatef(b->cx, b->cy, b->cz);
			glRotatef(b->heading, 0.0f, 1.0f, 0.0f);
			glCallList(b->ury_obj);
			glPopMatrix();
			break;
		default:
...
void makeCity(int x, int y, int size, gameState* gs) {
	...
	setColor(grey8);
	for (r = y; r <= y+size; r += 4000) {
		draw_rect(x, r, size, 300);
	}
	for (r = x; r <= x+size; r += 4000) {
		draw_rect(r, y, 300, size);
	}
}
void set_truck_route(int cx, int cy, int cz, float speed, int route_count, const int *route_x, const int *route_z, gameState *gs) {
	Building b;
	int i;
	b = new_building(TYPE_GROUND_VEHICLE, gs);
	b->cx = cx;
	b->cy = cy;
	b->cz = cz;
	b->fx = (float)cx;
	b->fz = (float)cz;
	b->speed = speed;
	b->route_count = route_count > 8 ? 8 : route_count;
	b->route_index = 1 % b->route_count;
	for (i = 0; i < b->route_count; i++) {
		b->route_x[i] = route_x[i];
		b->route_z[i] = route_z[i];
	}
	b->heading = 0.0f;
	b->vx = 0.0f;
	b->vz = 0.0f;
...
void update_ground_vehicles(gameState *gs) {
	int i;
	for (i = 0; i < current_building; i++) {
		Building b = gs->buildings[i];
		float dx, dz, distance;
		float target_heading;
		if (!b || b->type != TYPE_GROUND_VEHICLE || b->route_count < 2) {
			continue;
		}
		dx = (float)b->route_x[b->route_index] - b->fx;
		dz = (float)b->route_z[b->route_index] - b->fz;
		distance = sqrtf(dx * dx + dz * dz);
		if (distance < 120.0f) {
			b->route_index = (b->route_index + 1) % b->route_count;
			dx = (float)b->route_x[b->route_index] - b->fx;
			dz = (float)b->route_z[b->route_index] - b->fz;
			distance = sqrtf(dx * dx + dz * dz);
		}
		if (distance > 0.0f) {
			b->vx = dx / distance * b->speed;
			b->vz = dz / distance * b->speed;
			target_heading = atan2f(dx, -dz) * 180.0f / (float)M_PI;
			if (target_heading < 0.0f) target_heading += 360.0f;
			if (target_heading - b->heading > 180.0f) target_heading -= 360.0f;
			if (b->heading - target_heading > 180.0f) target_heading += 360.0f;
			b->heading += (target_heading - b->heading) * 0.12f;
		}
		b->fx += b->vx;
		b->fz += b->vz;
