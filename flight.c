...
static void apply_view_perspective(gameState *gs, float fov) {
...
static int plane_draw_object(const Plane pp) {
	return (pp->type >= C150 && pp->type <= HELICOPTER) ? pp->type : F18;
}

static void draw_plane_model(const Plane pp) {
	glPushMatrix();
	...
	glCallList(plane_draw_object(pp));
	glPopMatrix();
}

static float debug_effective_qs(const plane *pp) {
	return (pp->spoilers > 0) ? (pp->qs * pp->Splf) : pp->qs;
}
...
void handle_key(char k, gameState *gs, plane *pp) {
	...
	case 'x':
		...
		if (gs->view_switch == TOWER) {
			...
			apply_view_perspective(gs, gs->tower_fov);
		}
		/**/
		break;
	case 'D':
	...
	case 'G':
		if (gs->number_planes > 1) {
			gs->tower_target_index = (gs->tower_target_index + 1) % gs->number_planes;
		} else {
			gs->tower_target_index = 0;
		}
		break;
	case 'd':
		if (gs->view_switch == PILOTE) {
			...
			apply_view_perspective(gs, 45.0f);
		}
		break;
	case 'X':
		// free camera
	...
	case 'v':
	case 'V':
		pp->autopilot = !pp->autopilot;
		if (pp->autopilot) {
			pp->target_speed = pp->airspeed;
			pp->target_climb = pp->climbspeed;
			pp->target_heading = (short)pp->azimuthf;
			pp->target_altitude = pp->y;
		}
		break;
	case 'f':
	case 'F':
	...
	case 'l':
		if (pp->landing_gear_stuck != pp->wheels) {
			if (pp->wheels) /* bring wheels up	*/
			{
				...
				else if (pp->type == BALLOON || pp->type == BLIMP || pp->type == HELICOPTER) {
					pp->wheels = 0;
				}
				pp->wheels = FALSE;
				pp->Cdp /= 2.0;
...
void set_f18(struct gameState* gs, Plane pp) {
	pp->type = F18;
...
void set_balloon(struct gameState* gs, struct plane* pp) {
	pp->type = BALLOON;
	pp->LmaxDEF = 2.0f;
	pp->LminDEF = -1.0f;
	pp->Fmax = 0.0f;
...
void set_blimp(struct gameState* gs, struct plane* pp) {
	set_c150(gs, pp);
	pp->type = BLIMP;
	pp->obj = BLIMP;
	pp->plane_type = BLIMP_NAME;
	pp->W = 1000.0f;
	pp->fuel_weight = 0.0f;
	pp->Mthrust = 300.0f;
	pp->MAX_RK = 0;
...
void set_helicopter(struct gameState* gs, struct plane* pp) {
	set_c150(gs, pp);
	pp->type = HELICOPTER;
	pp->obj = HELICOPTER;
	pp->plane_type = HELICOPTER_NAME;
	pp->W = 2500.0f;
	pp->fuel_weight = 0.0f;
	pp->Mthrust = 1200.0f;
	pp->MAX_RK = 0;
...
static float wrap_heading_error(float current, float target) {
	float e = target - current;
	while (e > 1800.0f) e -= 3600.0f;
	while (e < -1800.0f) e += 3600.0f;
	return e;
}
...
static void apply_autopilot(struct gameState* gs, struct plane* pp) {
	float heading_error = wrap_heading_error(pp->azimuthf, pp->target_heading);
	float speed_error = (float)pp->target_speed - (float)pp->airspeed;
	/* altitude hold: only a very gentle elevator nudge, applied every 4 ticks
	 * to avoid over-correction and stall */
	if (pp->type == BALLOON || pp->type == BLIMP) {
		float altitude_error = pp->target_altitude - pp->y;
		pp->rudder = clampf_local(heading_error / 2400.0f, -0.25f, 0.25f);
		pp->thrust += (int)clampf_local(altitude_error * 0.008f, -1.0f, 1.0f);
		if (pp->thrust < pp->min_throttle) pp->thrust = pp->min_throttle;
		if (pp->thrust > pp->max_throttle) pp->thrust = pp->max_throttle;
		return;
	}

	/* heading hold: small roll only, no rudder fighting */
	pp->rollers = clampf_local(heading_error / 80.0f, -18.0f, 18.0f);
	pp->rudder = 0.0f;

	/* altitude hold: gentle elevator only on even ticks to reduce oscillation */
	if ((gs->tick_counter & 3) == 0) {
		float altitude_error = pp->target_altitude - pp->y;
		float desired_climb = clampf_local(altitude_error * 0.015f, -8.0f, 8.0f);
		float climb_error = desired_climb - (float)pp->climbspeed;
		pp->elevator = clampf_local(climb_error * 0.003f, -1.5f, 1.5f);
	}

	/* speed hold: one thrust step every 8 ticks */
	if ((gs->tick_counter & 7) == 0) {
		pp->thrust += (int)clampf_local(speed_error / 80.0f, -1.0f, 1.0f);
		if (pp->thrust < pp->min_throttle) pp->thrust = pp->min_throttle;
		if (pp->thrust > pp->max_throttle) pp->thrust = pp->max_throttle;
	}
}

static void simulation_balloon(struct gameState* gs, struct plane* pp, int msx, int msy, int XMIDDLE, int YMIDDLE) {
	float yaw_input;
	float heading;
	float vx_target;
	float vz_target;
	...
	float desired_vy;
	float desired_speed;
	float turn_rate_limit;
	float bank_target;
	int thrust_baseline;
	(void)YMIDDLE;

	if (pp->autopilot) {
		apply_autopilot(gs, pp);
	}

	if (pp->type == HELICOPTER) {
		float pitch_input;
		float forward_factor;
		float lift_power;
		mouse_yaw = -(float)(msx - XMIDDLE) * 0.0050f;
		mouse_yaw = clampf_local(mouse_yaw, -0.85f, 0.85f);
		pp->rudder = mouse_yaw;
		pp->elevator = clampf_local(-0.026f * (float)(msy - YMIDDLE), -1.0f, 1.0f);
		yaw_input = pp->rudder * 9.0f;
		pp->azimuth_speedf += (yaw_input - pp->azimuth_speedf) * 0.12f;
		pp->azimuth_speedf = clampf_local(pp->azimuth_speedf, -12.0f, 12.0f);
		bank_target = clampf_local(-pp->azimuth_speedf * 2.8f, -18.0f, 18.0f);
		pp->twist += (bank_target - pp->twist) * 0.18f;
		desired_pitch = clampf_local(-pp->elevator * 34.0f, -42.0f, 42.0f);
		pp->elevationf += (desired_pitch - pp->elevationf) * 0.26f;
		pitch_input = -pp->elevationf / 30.0f;
		forward_factor = clampf_local(pitch_input, 0.0f, 1.25f);
		lift_power = (float)pp->thrust * 0.040f;
		desired_vy = lift_power * (1.0f - 0.78f * forward_factor) - 1.15f + pp->elevator * 0.08f;
		pp->vy += (desired_vy - pp->vy) * 0.12f;
		heading = (pp->azimuthf * 0.1f) * (float)M_PI / 180.0f;
		desired_speed = lift_power * (0.02f + 0.26f * forward_factor);
		vx_target = sinf(heading) * desired_speed;
		vz_target = -cosf(heading) * desired_speed;
		pp->vx += (vx_target - pp->vx) * 0.14f;
		pp->vz += (vz_target - pp->vz) * 0.14f;
		if (pp->y <= 0.0f && desired_vy <= 0.0f) {
			pp->vy = 0.0f;
		}
	}
	else if (pp->type == BLIMP) {
		mouse_yaw = -(float)(msx - XMIDDLE) * 0.0025f;
		mouse_yaw = clampf_local(mouse_yaw, -0.35f, 0.35f);
		pp->rudder = mouse_yaw;
		pp->elevator = clampf_local(-0.01f * (float)(msy - YMIDDLE), -1.0f, 1.0f);
		turn_rate_limit = 4.0f;

		yaw_input = pp->rudder * 3.5f + mouse_yaw;
		pp->azimuth_speedf += (yaw_input - pp->azimuth_speedf) * 0.045f;
		pp->azimuth_speedf = clampf_local(pp->azimuth_speedf, -turn_rate_limit, turn_rate_limit);

		bank_target = clampf_local(-pp->azimuth_speedf * 2.2f, -6.0f, 6.0f);
		pp->twist += (bank_target - pp->twist) * 0.08f;

		desired_pitch = clampf_local(-pp->elevator * 14.0f, -10.0f, 10.0f);
		pp->elevationf += (desired_pitch - pp->elevationf) * 0.08f;

		desired_vy = clampf_local(pp->elevator * 0.22f, -0.22f, 0.22f);
		pp->vy += (desired_vy - pp->vy) * 0.04f;
		pp->vy *= 0.992f;

		thrust_baseline = 35;
		desired_speed = ((float)pp->thrust - (float)thrust_baseline) * 0.018f;
		if (desired_speed < 0.0f) desired_speed = 0.0f;

		setIdentity(pp->ptw);
		rotateM(pp->ptw, pp->azimuthf, 0, 1, 0);
		rotateM(pp->ptw, pp->elevationf, 1, 0, 0);
		rotateM(pp->ptw, pp->twist, 0, 0, 1);
		vx_target = -pp->ptw[2][0] * desired_speed;
		vz_target = -pp->ptw[2][2] * desired_speed;
		pp->vx += (vx_target - pp->vx) * 0.02f;
		pp->vz += (vz_target - pp->vz) * 0.02f;
	}
	else {
		/* Balloon controls are intentionally simple and damped.
		 * Thrust mainly controls vertical lift, not forward jet thrust.
		 */
		yaw_input = pp->rudder * 18.0f;
		pp->azimuth_speedf += (yaw_input - pp->azimuth_speedf) * 0.15f;
		pp->roll_speed *= 0.75f;
		pp->elevation_speedf *= 0.70f;
		pp->twist *= 0.85f;
		pp->elevationf *= 0.90f;

		/* Mouse x gives a very small bank/yaw hint, mostly for nudging. */
		pp->azimuth_speedf -= 0.02f * (float)(msx - XMIDDLE);
		if (pp->azimuth_speedf > 12.0f) pp->azimuth_speedf = 12.0f;
		if (pp->azimuth_speedf < -12.0f) pp->azimuth_speedf = -12.0f;

		/* Treat thrust like burner/lift control.
		 * Around 20 is a gentle hover, above climbs, below descends.
		 */
		desired_vy = ((float)pp->thrust - 20.0f) * 0.015f;
		pp->vy += (desired_vy - pp->vy) * 0.08f;
		pp->vy -= 0.0035f * gs->tps / TPS;

		desired_speed = ((float)pp->thrust > 20.0f) ? (((float)pp->thrust - 20.0f) * 0.01f) : 0.0f;
		heading = (pp->azimuthf * 0.1f) * (float)M_PI / 180.0f;
		vx_target = sinf(heading) * desired_speed;
		vz_target = -cosf(heading) * desired_speed;
		pp->vx += (vx_target - pp->vx) * 0.05f;
		pp->vz += (vz_target - pp->vz) * 0.05f;
	}

	pp->azimuthf += pp->azimuth_speedf;
	while (pp->azimuthf < 0.0f) pp->azimuthf += 3600.0f;
	while (pp->azimuthf >= 3600.0f) pp->azimuthf -= 3600.0f;
	...
	pp->airspeed = (int)(sqrtf(pp->vx * pp->vx + pp->vz * pp->vz) * pp->fps_knots);
...
void simulation(struct gameState* gs, struct plane* pp, int msx, int msy, int XMIDDLE, int YMIDDLE, int XMAXSCREEN, int YMAXSCREEN) {
	...
	gs->tick_counter++;
	if (pp->type == BALLOON || pp->type == BLIMP || pp->type == HELICOPTER) {
		simulation_balloon(gs, pp, msx, msy, XMIDDLE, YMIDDLE);
		return;
	}
...
void draw_game(gameState * gs, plane *pp) {
	...
	/* draw all planes once per frame using the current camera */
	draw_all_planes(gs);
